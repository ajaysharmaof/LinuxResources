...
main(){
  clear
  prHeader "="
  prtxtCenter "Welcome to git installation through bash shell script"
  prHeader "="
  prCyan "$(get_date_time) checking all available git versions from offical git-scm website. please wait.."

  download_git_versions_info
  diplay_all_available_git_versions
  display_existing_get_info
}

if [ "${USER}" != "root" ]
then
  prRed "$(get_date_time) please run this shell script with root user"
  exit 1
else
  main
fi

#$USER is enveroiment vareble in system
